...
class BaseRow(Row):
    ...
    def __init__(self, cursor: Cursor, fields: list[str], data: tuple[str]): ...
...
def row_factory(backend: SQLiteBackend) -> Callable[[Cursor, Row], BaseRow]: ...
...
class SQL:
    ...
    @staticmethod
    def operator_join(values: list[str], operator: str = Literal['and']) -> str: ...
...
class SQLiteBackend(SQL):
    ...
    def create_table_fields(self, table: Table, columns_to_create: list[str]): ...
    def list_tables_sql(self) -> list[BaseRow]: ...
    def list_database_indexes(self) -> List[BaseRow]: ...
    def list_table_indexes(self) -> List[BaseRow]: ...
    def save_row(self, row: BaseRow, updated_values: list[str]) -> List[BaseRow]: ...
